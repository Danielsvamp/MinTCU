...
public:
    ...
    uint32_t pClutchFromTorque(GearboxGear gear, Clutch clutch, uint16_t torque, clutchCoefType coefType);
...
uint32_t PressureManager::pClutchFromTorque(GearboxGear gear, Clutch clutch, uint16_t torque, clutchCoefType coefType) {
...
uint16_t PressureManager::calc_current_linear_sol(uint16_t p_targ, GearboxGear current_gear, GearChange change_state) {
    ...
    if (GearChange::_IDLE == change_state) { // Not shifting
        // Not shifting
        if (GearboxGear::First == current_gear || GearboxGear::ReverseFirst == current_gear) {
